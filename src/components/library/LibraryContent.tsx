...
export const LibraryContent: React.FC<LibraryContentProps> = ({
  ...
  // Fetch books with React Query - this already handles filtering server-side
  const {
    data: bookTemplates, // Renamed from allBookTemplates since this is already filtered
    isLoading: isBooksLoading,
    error: booksError,
  } = useQuery({
    queryKey: ["book-templates", { genres: selectedGenres }],
    queryFn: async () => {
      const result = await getAllBookTemplates({
        filterByGenre: selectedGenres,
        publishedOnly: true, // Make sure this matches your server component
      });
      if (!result.success) throw new Error(result.error);
      return result.data;
    },
    // Only use initialData if no genres are selected to avoid mismatch
    initialData: selectedGenres.length === 0 ? initialBookTemplates : undefined,
    staleTime: 60 * 1000 * 60,
  });
  ...
  // REMOVED: The useMemo client-side filtering since server-side filtering handles this
  ...
  const showEmptyState =
    !isBooksLoading && (!bookTemplates || bookTemplates.length === 0);
      ...
      <div className="bg-indigo-50/50 p-8 rounded-lg shadow-lg">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          >
            ...
            {!showEmptyState && (
              <BookTemplateGrid
                bookTemplates={bookTemplates || []} // Add fallback for undefined
                isLoading={isBooksLoading}
                hasActiveFilters={hasActiveFilters}
              />
            )}
