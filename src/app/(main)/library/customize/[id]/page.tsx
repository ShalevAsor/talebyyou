  ...
  try {
    ...
    if (templateResult.success) {
      ...
      return createMetadata({
        ...
        alternates: {
          canonical: `${siteConfig.url}/library/customize/${id}`,
        },
      });
    }
  } catch (error) {
...
export default async function BookCustomizePage({
  ...
  try {
    ...
    const bookTemplateResult = await getBookTemplateById(id);

    // Handle errors and not-found states
    if (!bookTemplateResult.success) {
      if (bookTemplateResult.error.includes("not found")) {
        logger.warn({ templateId: id }, "Template not found, showing 404 page");
        notFound();
      }

      logger.error(
        { error: bookTemplateResult.error, templateId: id },
