// src/app/api/test/test-disney-dragon/route.ts
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Step 4: Generate an image with character reference - Disney style dragon scene
    console.log(
      "Starting generation with character reference - Disney style dragon scene..."
    );

    // Using a dragon riding scene prompt with the structure from their examples but Disney style terms
    const result = await leonardo.image.createGeneration({
      height: 768,
      width: 768,
      modelId: "aa77f04e-3eec-4034-9c07-d0f619684628", // Leonardo Kino XL model
      prompt:
        "illustration of a child, riding a friendly colorful dragon, Disney animation style, vibrant colors, highly detailed, floating clouds, magical kingdom",
      numImages: 1,
      presetStyle: SdGenerationStyle.Render3D,
      public: false,
    ...
    return NextResponse.json({
      success: true,
      message:
        "Character reference generation started - Disney style dragon scene",
