...
import { BOOK_PRICES } from "@/types/pricing";
import { generateOrderNumber, getProductPrice } from "@/utils/orderUtils";
import { serializeOrder, serializeOrders } from "@/utils/serializers";

import { getPricingStatus } from "./pricing-actions";
import { getCurrentUser } from "./user-actions";

/**
 * Get an order by ID with related data
 * @returns the order with book and user relations
 */
...
export async function updateOrder(
  ...
  try {
    ...
    // Get current pricing and calculate total
    const pricingResult = await getPricingStatus();
    let totalPrice: Decimal;

    if (pricingResult.success) {
      const { pricing, discount, isDiscountActive } = pricingResult.data;
      const calculatedPrice = getProductPrice(
        productType,
        quantity,
        pricing,
      ...
      // Fallback to hardcoded prices if pricing system fails
...
export async function createOrRecoverOrder(
  ...
  try {
    ...
    // Create the order in the database with calculated price
    ...
    logger.info(
      ...
      "Successfully created order with dynamic pricing"
    );
