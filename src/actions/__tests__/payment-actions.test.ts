...
describe("Payment Actions", () => {
  ...
  describe("createPayPalOrder", () => {
    ...
    test("should create a PayPal order for a valid pending order", async () => {
      ...
      // Verify prisma and paypal service were called correctly
      expect(prisma.order.findUnique).toHaveBeenCalledWith({
        where: { id: "order123" },
      });
      expect(paypal.createOrder).toHaveBeenCalledWith(24.99);
  ...
  describe("processPayment", () => {
    // Create a separate implementation for each test to avoid conflicts
